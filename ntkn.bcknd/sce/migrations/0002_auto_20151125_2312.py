...
def load_initial_data(apps, schema_editor):
    ...
    with open('sce/initial_data/grade_levels.csv', 'rt', encoding='utf8') as f:
        ...
        for row in reader:
            print(row[2])
            educative_program = EducativeProgram.objects.get(slug=row[2])
            #educative_program = EducativeProgram.objects.get(slug='preescolar')
            print(educative_program)
        ...
        for row in reader:
            if reader.line_num == 1:
                ...
                subject = Subject(
                    ...
                    #is_active=True,
                    #description='',
                    grade_level=GradeLevel.objects.get(id=row[8]),
                    category=SubjectCategory.objects.get(pk=row[6]),
                    level=row[7],
                )
                subject.save()
                print('--added subject '+ subject.__str__())

    # Marking period
    print('\nLoading Marking Periods')
